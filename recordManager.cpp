...
index_info& recordManager::insert(string table_name, vector <attrAndvalue> &v_aav, vector<attrNode> &v_an){
						...
						switch (target_an.type){
							case 1:
								...
								if (temp_i == v_aav[t].int_value){
									...
									insert_ii.num = -1;	insert_ii.offset = -1;
									return insert_ii;
								}
								break;
							case 2:
							{
								string temp_str;
								p_temp_bn->readBlock(temp_str, v_aav[t].length, 4+toOfst[t]+current_offset);
	...
	p_temp_bn = getBlockPointer(0, table_name, block_num);
	int current_offset = 0;
	while (current_offset + r_length < 4096){
		int delOrUnwrite;
		p_temp_bn->readBlock(delOrUnwrite, 4, current_offset);
		if (delOrUnwrite==2){
			...
			for (it_aav = v_aav.begin(); it_aav != v_aav.end(); it_aav++){
				...
				case 1:
					if (temp_aav.isNull){
						...
						p_temp_bn->writeBlock((int)0, 4, current_offset + inner_offset + 4);
					}
					else{
						p_temp_bn->writeBlock((int)0, 4, current_offset + inner_offset);
						p_temp_bn->writeBlock(temp_aav.int_value, 4, current_offset + inner_offset + 4);
					}
				...
				case 2:
				{
						  ...
						  if (temp_aav.isNull){
							  p_temp_bn->writeBlock((int)1, 4, current_offset + inner_offset);
							  p_temp_bn->writeBlock(temp_str, temp_aav.length, current_offset + inner_offset + 4);
						  }
						  else{
							  p_temp_bn->writeBlock((int)0, 4, current_offset + inner_offset);
							  p_temp_bn->writeBlock(temp_aav.str_value, temp_aav.length, current_offset + inner_offset + 4);
						  }
				...
				case 3:
					if (temp_aav.isNull){
						p_temp_bn->writeBlock((int)1, 4, current_offset + inner_offset);
						p_temp_bn->writeBlock((float)0, 4, current_offset + inner_offset + 4);
					}
					else{
						p_temp_bn->writeBlock((int)0, 4, current_offset + inner_offset);
						p_temp_bn->writeBlock((float)temp_aav.float_value, 4, current_offset + inner_offset + 4);
					}
					inner_offset += 8;
					break;
				default:
			...
			if (delOrUnwrite == 1){
				...
				if ((current_offset + 2 * r_length) < 4096){
					...
					p_temp_bn = getBlockPointer(0, table_name, block_num+1);
					p_temp_bn->writeBlock((int)2, 4, 0);
					p_temp_bn = getBlockPointer(0, table_name, 0);
